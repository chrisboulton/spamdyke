 ...
 */
int nihdns_expand(char *answer_start, char *answer_end, char *answer_ptr, char *return_buf, int length_return_buf)
  {
  int return_value;
  char *tmp_ptr;
  ...
  if ((answer_start != NULL) &&
    ...
    while ((tmp_ptr >= answer_start) &&
      ...
      if ((tmp_ptr[0] & 0xC0) == 0xC0)
        {
        if (((tmp_ptr = answer_start + ((tmp_ptr[0] & 0x3F) << 6) + tmp_ptr[1]) > answer_end) ||
...
int nihdns_query(struct filter_settings *current_settings, char **target_name_array, int num_types, int *types, char *return_answer, int return_answer_length, char **return_answer_start, int *return_target_name_index)
  ...
  if ((udp_socket = nihdns_initialize(current_settings, 0)) != -1)
    {
    nihdns_empty_udp_buffer(udp_socket);
...
int nihdns_rbl(struct filter_settings *current_settings, char **target_name_array, char *target_message_format, char *target_message_buf, int size_target_message_buf, char **target_rbl_array, int *return_target_name_index, struct previous_action *history)
  ...
  if ((target_name_array != NULL) &&
      ...
      (answer_length = nihdns_query(current_settings, target_name_array, sizeof(types) / sizeof(int), types, answer, MAX_DNS_PACKET_BYTES_UDP, &answer_ptr, &target_name_index)) > 0)
    {
    num_answers = ntohs((unsigned short)((struct nihdns_header *)&answer)->ancount);
    num_cnames = 0;
      ...
      if ((cname_array = (char **)malloc(sizeof(char *) * (num_cnames + 1))) != NULL)
        ...
        for (i = 0; (i < num_cnames) && !return_value; i++)
          if ((size = nihdns_expand(answer, answer + answer_length, cname_ptr[i], (char *)host, MAX_HOSTNAME)) >= 0)
            if ((cname_array[j] = (char *)malloc(sizeof(char) * (size + 1))) != NULL)
              {
              num_queries = 0;
              ...
              {
              SPAMDYKE_LOG_ERROR(current_settings, LOG_ERROR_MALLOC, sizeof(char) * (size + 1));
              return_value = -1;
              break;
              }
          else
        ...
        if (cname_array[0] != NULL)
          return_value = nihdns_rbl(current_settings, cname_array, target_message_format, target_message_buf, size_target_message_buf, target_rbl_array, return_target_name_index, &current_lookup);
...
int nihdns_ptr_lookup(struct filter_settings *current_settings, char *target_name, struct previous_action *history)
  ...
  if ((target_name != NULL) &&
      ((answer_length = nihdns_query(current_settings, target_name_array, sizeof(types) / sizeof(int), types, answer, MAX_DNS_PACKET_BYTES_UDP, &answer_ptr, NULL)) > 0))
    {
    num_answers = ntohs((unsigned short)((struct nihdns_header *)&answer)->ancount);
    num_cnames = 0;
        ...
        switch (type)
          ...
          case NIHDNS_TYPE_PTR:
            if ((size = nihdns_expand(answer, answer + answer_length, answer_ptr, (char *)host, MAX_HOSTNAME)) > 0)
              ...
              current_settings->strlen_server_name = size;
              for (i = 0; i < size; i++)
  ...
  if ((target_name != NULL) &&
    ...
    for (i = 0; (i < num_cnames) && !return_value; i++)
      if ((size = nihdns_expand(answer, answer + answer_length, cname_ptr[i], (char *)host, MAX_HOSTNAME)) >= 0)
        ...
        SPAMDYKE_LOG_EXCESSIVE(current_settings, LOG_DEBUGX_DNS_CNAME, target_name, host);
        return_value = ((tmp_lookup == NULL) && (num_queries < MAX_DNS_QUERIES)) ? nihdns_a(current_settings, (char *)host, return_octets, &current_lookup, disqualify_localhost) : 0;
        answer_ptr += size;
        }
      else
        {
