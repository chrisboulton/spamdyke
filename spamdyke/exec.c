 ...
 */
int exec_command_argv(struct filter_settings *current_settings, char *filename, char *argv[], struct expect_send *protocol, char **return_content, int size_return_content)
  {
  int return_value;
  int i;
  int last_match;
            ...
            if ((child_pid = fork()) > 0)
              ...
              while ((max_fd >= 0) &&
                ...
                if ((protocol != NULL) &&
                    ...
                    FD_ISSET(output_pipe[1], &write_fds))
                  {
                  write(output_pipe[1], protocol[current_protocol].data, protocol[current_protocol].strlen_data);
                  current_protocol++;
                  }
 ...
 *   SUCCESS: length of returned string
 */
int exec_command(struct filter_settings *current_settings, char *command_line, struct expect_send *protocol, char **return_content, int size_return_content)
  {
  int return_value;
  int i;
  int j;
  ...
  if (command_line != NULL)
    ...
    if ((tmp_command_line = (char *)malloc(sizeof(char) * (strlen_command_line + 1))) != NULL)
      ...
      if ((child_argv = (char **)malloc(sizeof(char *) * (argc + 1))) != NULL)
        ...
        return_value = exec_command_argv(current_settings, child_argv[0], child_argv, protocol, return_content, size_return_content);
