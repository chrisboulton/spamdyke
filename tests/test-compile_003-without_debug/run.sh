# This test configures and compiles spamdyke without debug output to check for
...
pushd ${SPAMDYKE_DIR}
make distclean
echo "./configure --without-debug-output > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1"
./configure --without-debug-output > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1
