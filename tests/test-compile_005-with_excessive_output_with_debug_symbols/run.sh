...
pushd ${SPAMDYKE_DIR}
make distclean
echo "./configure --with-debug-symbols --with-excessive-output > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1"
./configure --with-debug-symbols --with-excessive-output > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1
