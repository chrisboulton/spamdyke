...
# contains the IP address and a country code.  The config directive comes
# from a recursively included configuration file.
...
echo "config-file=${TMPDIR}/${TEST_NUM}-config.txt" > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/example
echo "config-file=${TMPDIR}/${TEST_NUM}-config2.txt" > ${TMPDIR}/${TEST_NUM}-config.txt
echo "reject-ip-in-cc-rdns=yes" > ${TMPDIR}/${TEST_NUM}-config2.txt
echo "config-file=${TMPDIR}/${TEST_NUM}-config.txt" >> ${TMPDIR}/${TEST_NUM}-config2.txt
