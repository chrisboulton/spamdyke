...
# that matches the partial sender domain.
...
echo policy-url=foo > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/foo
echo policy-url=bar > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/bar
echo policy-url=baz > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/baz
echo reject-unresolvable-rdns > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/example
