...
# that matches the recipient domain (no username).
...
recipient_path=${recipient_dir}/_at_/`echo $1 | sed -e "s/@.*//"`

mkdir -p ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_dir}/_at_
