...
# that matches the partial recipient domain (no username).
...
recipient_dir=""
for segment in `echo $1 | awk '{ print tolower($1) }' | sed -e "s/[^@]*@//" | ${DOMAINSPLIT_PATH} | sed -e "s/\./ /g"`
do
  if [ "${recipient_dir}" == "" ]
  then
...
mkdir -p ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_dir}
...
echo reject-unresolvable-rdns > ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_path}
