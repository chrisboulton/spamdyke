...
# that matches the full sender address and the full recipient address.
...
mkdir -p ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/example/_at_/${FROM_USERNAME}/_recipient_/${recipient_dir}
...
echo reject-unresolvable-rdns > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/example/_at_/${FROM_USERNAME}/_recipient_/${recipient_path}
