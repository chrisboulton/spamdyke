...
# that matches the partial rDNS name, the partial sender address and the partial
# recipient address.
...
mkdir -p ${TMPDIR}/${TEST_NUM}-config.d/_rdns_/${rdns_path}/_sender_/com/example/_recipient_/${recipient_dir}
...
echo reject-unresolvable-rdns > ${TMPDIR}/${TEST_NUM}-config.d/_rdns_/${rdns_path}/_sender_/com/example/_recipient_/${recipient_path}
