...
# that matches the partial sender address and the partial rDNS name.
...
mkdir -p ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/example/_rdns_/${rdns_dir}
...
echo policy-url=dino > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/dino
echo policy-url=pebbles > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/pebbles
echo policy-url=bambam > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/bambam
echo reject-unresolvable-rdns > ${TMPDIR}/${TEST_NUM}-config.d/_sender_/com/example/_rdns_/${rdns_path}
