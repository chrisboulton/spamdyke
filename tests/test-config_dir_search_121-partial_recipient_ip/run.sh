...
# that matches the partial recipient address and the first 3 octets of the IP address.
...
mkdir -p ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_path}/_ip_/${octet_1}/${octet_2}
...
echo policy-url=dino > ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_dir}/dino
echo policy-url=pebbles > ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_dir}/pebbles
echo policy-url=bambam > ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_dir}/bambam
echo reject-unresolvable-rdns > ${TMPDIR}/${TEST_NUM}-config.d/_recipient_/${recipient_path}/_ip_/${octet_1}/${octet_2}/${octet_3}
