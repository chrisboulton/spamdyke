...
echo rejection-text-sender-blacklist=Foo Bar Baz >> ${TMPDIR}/${TEST_NUM}-config.txt
echo sender-blacklist-file=${TMPDIR}/${TEST_NUM}-sender_blacklist.txt >> ${TMPDIR}/${TEST_NUM}-config.txt
