...
# non-executable file owned by the user.
...
output=`grep "File is not executable: ${TMPDIR}/${TEST_NUM}-hostname: Owner permissions apply but owner executable bit is not set" ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
