...
# non-executable file owned by the user's primary group.

touch ${TMPDIR}/${TEST_NUM}-hostname
chown 1 ${TMPDIR}/${TEST_NUM}-hostname
chgrp `groups | awk '{ print $1 }'` ${TMPDIR}/${TEST_NUM}-hostname
...
output=`grep "File is not executable: ${TMPDIR}/${TEST_NUM}-hostname: Group permissions apply but group executable bit is not set" ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
