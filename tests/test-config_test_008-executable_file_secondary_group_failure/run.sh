...
chgrp `groups | awk '{ print $2 }'` ${TMPDIR}/${TEST_NUM}-hostname
...
output=`grep "File is not executable: ${TMPDIR}/${TEST_NUM}-hostname: Group permissions apply (through a secondary group) but group executable bit is not set" ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
