...
# unwritable domain directory.

if [ "${UID}" == "0" ]
then
  mkdir -p ${TMPDIR}/${TEST_NUM}-graylist.d/example.com
  chmod 555 ${TMPDIR}/${TEST_NUM}-graylist.d/example.com

  pushd ..
  su $4 ./subrun $1 $2 $3 $4 $5 test-${TEST_NUM}-*
  popd

  output=`grep "Failed to create file in directory: ${TMPDIR}/${TEST_NUM}-graylist.d/example.com/" ${TMPDIR}/${TEST_NUM}-output.txt`
  if [ ! -z "${output}" ]
  then
