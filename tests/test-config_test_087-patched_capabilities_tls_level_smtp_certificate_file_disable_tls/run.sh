...
# and an SSL certificate is given.
...
then
  ...
  if [ -z "${output}" ]
  then
    echo "${SPAMDYKE_PATH} -lverbose --tls-level=smtp --tls-certificate-file ${CERTDIR}/combined_no_passphrase/server.pem --config-test ${QMAIL_CMDLINE} >> ${TMPDIR}/${TEST_NUM}-output.txt 2>&1"
    ${SPAMDYKE_PATH} -lverbose --tls-level=smtp --tls-certificate-file ${CERTDIR}/combined_no_passphrase/server.pem --config-test ${QMAIL_CMDLINE} >> ${TMPDIR}/${TEST_NUM}-output.txt 2>&1

    output=`grep "ERROR: ${child_cmd} appears to offer TLS support. The \"tls-type\" and \"tls-certificate-file\" options are being used but TLS support is not compiled into spamdyke. Unless it is recompiled with TLS support, the following spamdyke features will not function during TLS deliveries: graylisting, sender whitelisting, sender blacklisting, sender domain MX checking, DNS RHSBL checking for sender domains, recipient whitelisting, recipient blacklisting, limited number of recipients and full logging." ${TMPDIR}/${TEST_NUM}-output.txt`
    if [ ! -z "${output}" ]
    then
