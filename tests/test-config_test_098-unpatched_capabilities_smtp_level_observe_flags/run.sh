...
# process does not support SMTP AUTH, smtp-auth-level is "observe" and
...
if [ ! -z "${output}" ]
then
  output=`grep "ERROR: ${child_cmd} does not appear to offer SMTP AUTH support. spamdyke will observe any authentication and trust its response (although that appears unlikely to happen). The \"smtp-auth-command\" option was given but will be ignored." ${TMPDIR}/${TEST_NUM}-output.txt`
  if [ ! -z "${output}" ]
  then
    outcome="success"
  else
