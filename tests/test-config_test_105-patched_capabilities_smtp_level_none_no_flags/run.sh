...
# process supports SMTP AUTH and the smtp-auth-level option is "none".
...
output=`grep "SUCCESS: ${child_cmd} appears to offer SMTP AUTH support but spamdyke will block authentication attempts." ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
  outcome="success"
else
