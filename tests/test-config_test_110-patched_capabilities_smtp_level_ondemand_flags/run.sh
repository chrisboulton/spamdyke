...
# process supports SMTP AUTH, smtp-auth-level is "ondemand" and
...
output=`grep "SUCCESS: ${child_cmd} appears to offer SMTP AUTH support. spamdyke will observe any authentication and trust its response. spamdyke will offer authentication if ${child_cmd} does not." ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
  outcome="success"
else
