...
# contains a file named _at_.

mkdir -p ${TMPDIR}/${TEST_NUM}-config.d
touch ${TMPDIR}/${TEST_NUM}-config.d/_at_
...
output=`grep "ERROR(config-dir): Found a file named \"_at_\", which should only be used for directory names. This file name is invalid and will be ignored. Full path: ${TMPDIR}/${TEST_NUM}-config.d/_at_" ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
  outcome="success"
else
