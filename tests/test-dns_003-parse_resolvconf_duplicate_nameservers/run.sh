...
# nameserver.  The nameserver value is repeated several times.  spamdyke
...
if [ ! -z "${output}" ]
then
  output=`grep "discarded duplicate nameserver found at /etc/resolv.conf" ${TMPDIR}/${TEST_NUM}-output.txt | wc -l | awk '{ print $1 }'`
  if [ "${output}" == "2" ]
  then
    output=`grep "for 44.33.22.11.in-addr.arpa(PTR) to DNS server ${NAMESERVER_IP}:53 (attempt 1)" ${TMPDIR}/${TEST_NUM}-output.txt | wc -l | awk '{ print $1 }'`
    if [ "${output}" == "1" ]
    then
      outcome="success"
    else
