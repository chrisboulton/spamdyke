...
# nameserver.  The nameserver values include invalid port numbers.  spamdyke
# should discard the bad numbers and use the default port.
...
then
  ...
  then
    ...
    if [ ! -z "${output}" ]
    then
      output=`grep "ERROR: invalid/unparsable nameserver port number found, defaulting to 53 instead: ${NAMESERVER_IP_B}" ${TMPDIR}/${TEST_NUM}-output.txt`
      if [ ! -z "${output}" ]
      then
        output=`grep "found nameserver: ${IP_A}:53" ${TMPDIR}/${TEST_NUM}-output.txt | wc -l | awk '{ print $1 }'`
        if [ "${output}" == "1" ]
        then
          output=`grep "found nameserver: ${IP_B}:53" ${TMPDIR}/${TEST_NUM}-output.txt | wc -l | awk '{ print $1 }'`
          if [ "${output}" == "1" ]
          then
            output=`grep "found nameserver: ${NAMESERVER_IP_C}:53" ${TMPDIR}/${TEST_NUM}-output.txt | wc -l | awk '{ print $1 }'`
            if [ "${output}" == "1" ]
            then
              outcome="success"
            else
