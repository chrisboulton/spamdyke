...
# nameserver.  The default port number is given with an invalid value.
# spamdyke should ignore the bad value and use the default port number.
...
then
  ...
  then
    ...
    if [ ! -z "${output}" ]
    then
      output=`grep "ERROR: invalid/unparsable default port found in file /etc/resolv.conf on line 4: port asdf" ${TMPDIR}/${TEST_NUM}-output.txt`
      if [ ! -z "${output}" ]
      then
