...
# nameserver.  An invalid total timeout value is given, which spamdyke should
...
cat resolv_conf.txt | sed -e "s/NAMESERVER_IP/${NAMESERVER_IP}/g" -e "s/TIMEOUT/foo/g" > /etc/resolv.conf
...
then
  ...
  if [ ! -z "${output}" ]
  then
    output=`grep "ERROR: invalid/unparsable total timeout found in file /etc/resolv.conf on line 2: timeout foo" ${TMPDIR}/${TEST_NUM}-output.txt`
    if [ ! -z "${output}" ]
    then
      outcome="success"
    else
