# Too many secondary nameserver values are given.  spamdyke should use only
...
echo "${SENDRECV_PATH} -t 30 -r 221 -- ${SPAMDYKE_PATH} -lexcessive --log-target stderr --dns-server-ip ${NAMESERVER_IP_A} --dns-server-ip ${NAMESERVER_IP_B} --dns-server-ip ${NAMESERVER_IP_C} --dns-server-ip ${NAMESERVER_IP_D} --dns-server-ip ${NAMESERVER_IP_E} --dns-server-ip ${NAMESERVER_IP_F} --dns-server-ip ${NAMESERVER_IP_G} --dns-server-ip ${NAMESERVER_IP_H} --dns-server-ip ${NAMESERVER_IP_I} --dns-server-ip ${NAMESERVER_IP_J} --dns-server-ip ${NAMESERVER_IP_K} --dns-server-ip ${NAMESERVER_IP_L} --dns-server-ip ${NAMESERVER_IP_M} --dns-server-ip ${NAMESERVER_IP_N} --dns-server-ip ${NAMESERVER_IP_O} --dns-server-ip ${NAMESERVER_IP_P} --dns-server-ip ${NAMESERVER_IP_Q} --dns-server-ip ${NAMESERVER_IP_R} ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1"
${SENDRECV_PATH} -t 30 -r 221 -- ${SPAMDYKE_PATH} -lexcessive --log-target stderr --dns-server-ip ${NAMESERVER_IP_A} --dns-server-ip ${NAMESERVER_IP_B} --dns-server-ip ${NAMESERVER_IP_C} --dns-server-ip ${NAMESERVER_IP_D} --dns-server-ip ${NAMESERVER_IP_E} --dns-server-ip ${NAMESERVER_IP_F} --dns-server-ip ${NAMESERVER_IP_G} --dns-server-ip ${NAMESERVER_IP_H} --dns-server-ip ${NAMESERVER_IP_I} --dns-server-ip ${NAMESERVER_IP_J} --dns-server-ip ${NAMESERVER_IP_K} --dns-server-ip ${NAMESERVER_IP_L} --dns-server-ip ${NAMESERVER_IP_M} --dns-server-ip ${NAMESERVER_IP_N} --dns-server-ip ${NAMESERVER_IP_O} --dns-server-ip ${NAMESERVER_IP_P} --dns-server-ip ${NAMESERVER_IP_Q} --dns-server-ip ${NAMESERVER_IP_R} ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1
