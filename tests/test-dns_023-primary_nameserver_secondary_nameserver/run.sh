# This test passes a primary nameserver IP and a secondary nameserver IP to
...
then
  ...
  if [ -z "${output}" ]
  then
    output=`grep "for 44.33.22.11.in-addr.arpa(PTR) to DNS server ${NAMESERVER_IP_SECONDARY} (attempt 2)" ${TMPDIR}/${TEST_NUM}-output.txt`
    if [ ! -z "${output}" ]
    then
      outcome="success"
    else
