# This test checks the number of DNS retries: 1 to the primary, 1 more
# to both.
...
${SENDRECV_PATH} -t 30 -r 221 -- ${SPAMDYKE_PATH} -lexcessive --log-target stderr --dns-timeout-secs 10 --dns-server-ip-primary ${NAMESERVER_PRIMARY_IP} --dns-server-ip ${NAMESERVER_SECONDARY_IP} --dns-max-retries-total 2 ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1
