# This test checks the number of DNS retries: 3 to the primary, 0 more
# to both because dns-max-retries-primary is greater than
...
${SENDRECV_PATH} -t 30 -r 221 -- ${SPAMDYKE_PATH} -lexcessive --log-target stderr --dns-timeout-secs 10 --dns-server-ip-primary ${NAMESERVER_PRIMARY_IP} --dns-server-ip ${NAMESERVER_SECONDARY_IP} --dns-max-retries-total 3 --dns-max-retries-primary 10 ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt 2>&1
