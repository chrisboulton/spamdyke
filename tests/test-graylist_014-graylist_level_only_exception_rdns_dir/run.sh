...
echo "${SENDRECV_PATH} -t 30 -r 421 -- ${SPAMDYKE_PATH} --graylist-level only --graylist-dir ${TMPDIR}/${TEST_NUM}-graylist.d --graylist-exception-rdns-dir ${TMPDIR}/${TEST_NUM}-grayblacklist.d ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt"
${SENDRECV_PATH} -t 30 -r 421 -- ${SPAMDYKE_PATH} --graylist-level only --graylist-dir ${TMPDIR}/${TEST_NUM}-graylist.d --graylist-exception-rdns-dir ${TMPDIR}/${TEST_NUM}-grayblacklist.d ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt
