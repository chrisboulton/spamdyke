...
mkdir -p ${TMPDIR}/${TEST_NUM}-graylist_three.d/quux.`echo $1 | sed -e "s/[^@]*@//" | awk '{ print tolower($1) }'`
...
then
  ...
  then
    ...
    then
      if [ ! -d ${TMPDIR}/${TEST_NUM}-graylist_three.d/`echo $1 | sed -e "s/[^@]*@//" | awk '{ print tolower($1) }'` ]
      then
        outcome="success"
      else
        echo "${TMPDIR}/${TEST_NUM}-graylist_three.d/`echo $1 | sed -e "s/[^@]*@//" | awk '{ print tolower($1) }'` exists but spamdyke should not have created it!"
        outcome="failure"
      fi
    else
    ...
    echo "${TMPDIR}/${TEST_NUM}-graylist_one.d/`echo $1 | sed -e "s/[^@]*@//" | awk '{ print tolower($1) }'` does not exist but spamdyke should have created it!"
    outcome="failure"
  fi
else
