...
then
  ...
  then
    ...
    then
      if [ -f ${TMPDIR}/${TEST_NUM}-graylist.d/example.com/${TO_USERNAME}/_none/_none ]
      then
        outcome="success"
      else
        echo ${TMPDIR}/${TEST_NUM}-graylist.d/example.com/${TO_USERNAME}/_none/_none does not exist or is not a file
        outcome="failure"
      fi
    else
