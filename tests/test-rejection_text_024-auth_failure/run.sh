...
echo "${SENDRECV_PATH} -t 30 -r 421 -- ${SPAMDYKE_PATH} --rejection-text-auth-failure \"Foo Bar Baz\" --smtp-auth-level always --smtp-auth-command \"${AUTH_CMDLINE}\" ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt"
${SENDRECV_PATH} -t 30 -r 421 -- ${SPAMDYKE_PATH} --rejection-text-auth-failure "Foo Bar Baz" --smtp-auth-level always --smtp-auth-command "${AUTH_CMDLINE}" ${QMAIL_CMDLINE} < ${TMPDIR}/${TEST_NUM}-input.txt > ${TMPDIR}/${TEST_NUM}-output.txt
