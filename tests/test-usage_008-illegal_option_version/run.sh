...
echo "version" > ${TMPDIR}/${TEST_NUM}-config.txt
...
output=`grep "ERROR: Option not allowed in configuration file, found in file ${TMPDIR}/${TEST_NUM}-config.txt on line 1: version" ${TMPDIR}/${TEST_NUM}-output.txt`
if [ ! -z "${output}" ]
then
  outcome="success"
else
