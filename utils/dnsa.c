...
#define MSG_SEARCHING           "Searching for A record: %s\n"
#define MSG_FOUND               "Found A record for %s: "
...
int dns_a(char *target_name, struct previous_action *history, int verbose)
  {
  int return_value;
  int i;
  unsigned char answer[PACKETSZ];
  ...
  if (target_name != NULL)
    ...
    if ((answer_length = dns_get(target_name, T_A, answer, PACKETSZ, verbose)) >= 0)
      {
          ...
          if ((size = dn_skipname(answer_ptr, answer + answer_length)) >= 0)
            ...
            if (type == T_A)
              ...
              if ((size = dn_expand(answer, answer + answer_length, answer_ptr, (char *)host, MAX_HOSTNAME)) >= 0)
                ...
                return_value = (tmp_lookup == NULL) ? dns_a((char *)host, &current_lookup, verbose) : 0;
                answer_ptr += size;
                }
              else
                {
...
void usage()
  ...
  printf(
    ...
    "\n"
    "Performs a DNS lookup for the A records associated with FQDN and prints the results.\n"
    "\n"
    "-v\n"
