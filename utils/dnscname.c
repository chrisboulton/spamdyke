...
#define MSG_SEARCHING           "Searching for CNAME record: %s\n"
#define MSG_FOUND               "Found CNAME record for %s: "
...
int dns_ns(char *target_name, struct previous_action *history, int verbose)
  ...
  if (target_name != NULL)
    ...
    if ((answer_length = dns_get(target_name, T_CNAME, answer, PACKETSZ, verbose)) >= 0)
      {
        ...
        for (i = 0; i < num_answers; i++)
          if ((size = dn_skipname(answer_ptr, answer + answer_length)) >= 0)
            ...
            if (type == T_CNAME)
              ...
              {
              printf(ERROR_TYPE, type, T_CNAME);
              break;
              }
            }
          else
...
void usage()
  ...
  printf(
    ...
    "\n"
    "Performs a DNS lookup for the CNAME records associated with FQDN and prints the results.\n"
    "\n"
    "-v\n"
