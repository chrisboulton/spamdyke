...
#define MSG_SEARCHING           "Searching for PTR record: %s\n"
#define MSG_FOUND               "Found PTR record for %s: "
...
int dns_ptr_lookup(char *target_name, struct previous_action *history, int verbose)
  {
  int return_value;
  int i;
  unsigned char answer[PACKETSZ];
  ...
  if (target_name != NULL)
    ...
    if ((answer_length = dns_get(target_name, T_PTR, answer, PACKETSZ, verbose)) >= 0)
      {
          ...
          if ((size = dn_skipname(answer_ptr, answer + answer_length)) >= 0)
            ...
            if (type == T_PTR)
              ...
              if ((size = dn_expand(answer, answer + answer_length, answer_ptr, (char *)host, MAX_HOSTNAME)) >= 0)
                ...
                return_value = (tmp_lookup == NULL) ? dns_ptr_lookup((char *)host, &current_lookup, verbose) : 0;
                answer_ptr += size;
                }
              else
                {
...
int dns_ptr(char *target_ip, int verbose)
  {
  int return_value;
  char ip_octets[4][4];
  ...
  if ((target_ip != NULL) &&
      (((sscanf(target_ip, "%3[0-9].%3[0-9].%3[0-9].%3[0-9]", ip_octets[0], ip_octets[1], ip_octets[2], ip_octets[3]) == 4) &&
       ...
       strncpy(rdns_name, target_ip, MAX_RDNS)))
    return_value = dns_ptr_lookup(rdns_name, NULL, verbose);
...
void usage()
  ...
  printf(
    ...
    "\n"
    "Performs a DNS lookup for the PTR records associated with FQDN and prints the results.\n"
    "If an IP address is given, it is reversed and " RDNS_SUFFIX " is appended to create FQDN.\n"
    "\n"
    "-v\n"
