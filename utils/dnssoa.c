...
#define MSG_SEARCHING           "Searching for SOA record: %s\n"
#define MSG_FOUND               "Found SOA record for %s:\n"
...
int dns_soa(char *target_name, struct previous_action *history, int verbose)
  {
  int return_value;
  int i;
  unsigned char answer[PACKETSZ];
  ...
  if (target_name != NULL)
    ...
    if ((answer_length = dns_get(target_name, T_SOA, answer, PACKETSZ, verbose)) >= 0)
      {
          ...
          if ((size = dn_skipname(answer_ptr, answer + answer_length)) >= 0)
            ...
            if (type == T_SOA)
              ...
              if ((size = dn_expand(answer, answer + answer_length, answer_ptr, (char *)host, MAX_HOSTNAME)) >= 0)
                ...
                return_value = (tmp_lookup == NULL) ? dns_soa((char *)host, &current_lookup, verbose) : 0;
                answer_ptr += size;
                }
              else
                {
...
void usage()
  ...
  printf(
    ...
    "\n"
    "Performs a DNS lookup for the SOA records associated with FQDN and prints the results.\n"
    "\n"
    "-v\n"
